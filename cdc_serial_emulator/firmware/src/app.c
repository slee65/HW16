...
//take com then add or subtract 320 then divide by 320 and move it up to 75% duty cycle

if ( COM>160 && COM<320){

    //int val= (float)(COM/(float)(320))*15000;
    //int val2=(float)(COM/(float)(320))*10000;
        OC1RS=6000;
        OC2RS=18000;
    //OC1RS=val2;
    //OC2RS=val;
...
else if (320<COM && COM<480){

    //int val= (float)((COM-320)/320)*15000;
    //int val2=(float)(320/(float)COM)*10000;
    //OC1RS=val;
    //OC2RS=val2;
...
else if (480<=COM && COM<=640) {
 OC1RS=15000;
 OC2RS=5000;

 }
 else if (0<=COM && COM<=160) {
 OC1RS=5000;
 OC2RS=15000;
  ...
  */
    ...
    */
          ...
          while(message[len]>0){
            ...
            if(USB_DEVICE_CDC_RESULT_OK != writeRequestResult) {BSP_LEDToggle(BSP_LED_3+7);
            }  // {turn on another led if there is an error } ;

                else //{ turn off the led if there is not an error };
                {BSP_LEDToggle(BSP_LED_3+7);
                }
